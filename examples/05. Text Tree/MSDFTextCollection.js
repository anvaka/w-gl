...
import {defineProgram, InstancedAttribute, GLCollection} from '../../build/wgl.module.js';
...
export default class MSDFTextCollection extends GLCollection {
  ...
  addText(textInfo) {
    ...
    for (let char of text) {
      ...
      const angle = textInfo.angle || 0;
      const dy = -sdfPos.yoffset * scale;
      const ox = x + dx;
      const oy = y + dy;
      // rotate ox, oy:
      ...
      this.add({
        position: [x + rx, y + ry, z],
        charSize: [
          (fontSize * sdfPos.width) / 42,
          (-fontSize * sdfPos.height) / 42,
          angle, //(Math.random() - 0.5) * 0.52 // angle
        ],
        texturePosition: [
...
function getTextProgram(gl, options) {
  ...
  void main() {
    vec3 pt = vec3(vec2(point.x, point.y) * charSize.xy, position.z);
    // rotate pt:
    float angle = charSize.z;
    pt = vec3(pt.x * cos(angle) - pt.y * sin(angle), pt.x * sin(angle) + pt.y * cos(angle), pt.z);

    vec4 p = vec4(position + pt, 1.);
